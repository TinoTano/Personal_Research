...
bool Collision::Update(float dt)
{
	...
	list<Collider*> possibleCollision;
	quadTreeChecks = 0;

	for (list<Collider*>::iterator it = colliders.begin(); it != colliders.end(); it++) {
		possibleCollision.clear();
		quadTree->Retrieve(possibleCollision, *it);
		quadTreeTimer.Start();
		if (possibleCollision.size() > 0) {
			for (list<Collider*>::iterator it2 = possibleCollision.begin(); it2 != possibleCollision.end(); it2++) {
	...
	quadTree->GetParentNodes(nodeList);
