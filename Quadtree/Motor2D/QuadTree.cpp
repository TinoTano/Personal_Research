...
{
	node[0] = node[1] = node[2] = node[3] = parent = nullptr;
	nodeIndex = 0;
	nodeRect = nodeBounds;
}
...
bool QuadTree::Intersects(Collider * col)
{
	return (bool)SDL_HasIntersection(&col->rect, &nodeRect);
}

void QuadTree::GetParentNodes(vector<QuadTree*>& nodeList)
{
	for (int i = 0; i < NODE_COUNT; i++) {
		if (node[i] != nullptr) {
			node[i]->GetParentNodes(nodeList);
		}
	}
